# Run this file for estimating model parameters for rat dataset
...
# df_rt = df_rt[df_rt["subjectID"] == 459]  # Only Rat #459
...
if __name__ == '__main__':
    ...
    with Pool(processes=cpu_nums) as pool:
        ...
        args = [(i, rat_sample, model_2, loss_function) for i in range(run_numbers)]
        all_models_2 = pool.starmap(calc_fit_model, args)

        args = [(i, rat_sample, model_3, loss_function) for i in range(run_numbers)]
        all_models_3 = pool.starmap(calc_fit_model, args)

        args = [(i, rat_sample, model_4, loss_function) for i in range(run_numbers)]
        all_models_4 = pool.starmap(calc_fit_model, args)

        args = [(i, rat_sample, model_1, loss_function) for i in range(run_numbers)]
