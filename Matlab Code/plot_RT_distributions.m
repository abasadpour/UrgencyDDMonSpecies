...
for model_case = 1 : num_of_models
    ...
    for coh = coherence
        ...
        for condition = 1 : length(conditions)
            ...
            if model_case == 1
                ...
                text_vertical_shift = max(hLF1.YLim) * 1.4;
    ...
    title(tcl(model_case),titles{case_number},'FontSize',title_font_size * 1.4, 'FontWeight',title_fontweight, 'visible', 'off')
    % Positioning the title at the top left
    currentAxes = ax(model_case, 1); % Get the handle to the first axes in the current row
    titleText = titles{case_number}; % Your title text
    xPos = min(xlim(currentAxes)) - 0.2; % X position at the start of the axes
    yPos = max(ylim(currentAxes)) * 1.3; % Y position slightly above the axes

    % Create a new title using the text function
    text(currentAxes, xPos, yPos, titleText, 'FontSize', title_font_size * 1.3, 'FontWeight', title_fontweight, 'HorizontalAlignment', 'left');
