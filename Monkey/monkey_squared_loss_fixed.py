# Run this file for estimating model parameters for monkey dataset
...
df_rt = df_rt[df_rt["rt"] < 1.65]  # Remove trials greater than 2500ms
...
driftcoh = Fittable(minval=0, maxval=20)
Sy = Fittable(minval=0.01, maxval=15)
Sx = Fittable(minval=0, maxval=20)
d = Fittable(minval=0, maxval=2)
sigma0 = Fittable(minval=0, maxval=5)
B = 1
nondectime = Fittable(minval=0.2, maxval=.8)
...
if __name__ == '__main__':
    cpu_nums = cpu_count() - 2
    run_numbers = cpu_nums
    ...
    with Pool(processes=cpu_nums) as pool:
        ...
        args = [(i, roitman_sample, model_2, loss_function) for i in range(run_numbers)]
        all_models_2 = pool.starmap(calc_fit_model, args)

        args = [(i, roitman_sample, model_3, loss_function) for i in range(run_numbers)]
        all_models_3 = pool.starmap(calc_fit_model, args)

        args = [(i, roitman_sample, model_4, loss_function) for i in range(run_numbers)]
        all_models_4 = pool.starmap(calc_fit_model, args)

        args = [(i, roitman_sample, model_1, loss_function) for i in range(run_numbers)]
    ...
    file_name = 'all_models_1_' + loss_function_name + '.pkl'
    with open(file_name, 'wb') as file:
        pickle.dump(all_models_1, file)
        print(f'Object successfully saved to "{file_name}"')

    file_name = 'all_models_2_' + loss_function_name + '.pkl'
    with open(file_name, 'wb') as file:
        pickle.dump(all_models_2, file)

        print(f'Object successfully saved to "{file_name}"')

    file_name = 'all_models_3_' + loss_function_name + '.pkl'
    with open(file_name, 'wb') as file:
        pickle.dump(all_models_3, file)

        print(f'Object successfully saved to "{file_name}"')

    file_name = 'all_models_4_' + loss_function_name + '.pkl'
    with open(file_name, 'wb') as file:
